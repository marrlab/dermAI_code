...
class DataGenerator(object):
    def __init__(self, batch_size=1, shuffle=False, patch_size = 512):
    ...
    def __Data_Genaration(self, batch_train):
        ...
        for ibatch, batch in enumerate(batch_train):
            aug_batch = []
            img_data = batch[0]#np.asarray(imageio.imread(batch[0]), dtype=np.float32)
            #print(img_data[0])
            #if isinstance(img_data[0][0], str):
            for i in img_data:
                img = i
    ...
    def generate(self, train_set,knn=False):
    ...
    def data_generation(self,batch_train):
        bag_batch = []
        bag_label = []
